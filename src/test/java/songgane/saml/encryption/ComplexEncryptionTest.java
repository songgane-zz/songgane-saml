...
package songgane.saml.encryption;
...
/**
 * Tests for encryption using single and multicast key encryption keys, and peer
 * vs. inline key placement.
 */
public class ComplexEncryptionTest extends BaseTestCase {
	...
	/**
	 * Test encryption with a single key encryption key with key placement
	 * inline.
	 */
	@Test
	public void testSingleKEKInline() {
		...
		assertEquals("Number of peer EncryptedKeys", 0, encTarget.getEncryptedKeys().size());
		...
		assertEquals("KeyName", expectedKeyNameRSA, encKey.getKeyInfo().getKeyNames().get(0).getValue());

		assertFalse("EncryptedKey ID attribute was empty", DatatypeHelper.isEmpty(encKey.getID()));
	...
	/**
	 * Test encryption with a single key encryption key with key placement as
	 * peer.
	 */
	public void testSingleKEKPeer() {
		...
		assertEquals("Number of inline EncryptedKeys", 0,
				encTarget.getEncryptedData().getKeyInfo().getEncryptedKeys().size());
		assertEquals("Number of peer EncryptedKeys", 1, encTarget.getEncryptedKeys().size());
		...
		assertEquals("EncryptedData RetrievalMethod had incorrect URI value", "#" + encKey.getID(), rm.getURI());
		...
		assertEquals("EncryptedKey DataReference had incorrect URI value", "#" + encData.getID(), dr.getURI());
		assertNull("EncryptedKey CarriedKeyName wasn't null", encKey.getCarriedKeyName());
	}

	/**
	 * Test encryption with multicast key encryption keys with key placement as
	 * peer.
	 */
	public void testMulticastKEKPeer() {
		...
		assertEquals("Number of inline EncryptedKeys", 0,
				encTarget.getEncryptedData().getKeyInfo().getEncryptedKeys().size());
		assertEquals("Number of peer EncryptedKeys", 2, encTarget.getEncryptedKeys().size());
		...
		assertEquals("Algorithm attribute", kekURIRSA, encKeyRSA.getEncryptionMethod().getAlgorithm());
		assertEquals("Algorithm attribute", kekURIAES, encKeyAES.getEncryptionMethod().getAlgorithm());

		assertFalse("EncryptedKey ID attribute was empty", DatatypeHelper.isEmpty(encKeyRSA.getID()));
		assertFalse("EncryptedKey ID attribute was empty", DatatypeHelper.isEmpty(encKeyAES.getID()));

		EncryptedData encData = encTarget.getEncryptedData();
		assertNotNull("EncryptedData KeyInfo wasn't null", encData.getKeyInfo());
		assertEquals("EncryptedData contained invalid number RetrievalMethods", 0,
				encData.getKeyInfo().getRetrievalMethods().size());
		assertEquals("EncryptedData contained invalid number KeyNames", 1, encData.getKeyInfo().getKeyNames().size());
